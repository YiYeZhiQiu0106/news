		...
		$.ajax({
			...
			success : function(response) {
				for(var i = 0; i<response.length; i++){
					$("#ul_type").append("<li><a href='${pageContext.request.contextPath}/b_newsManage/showNewsByType.action?type_id="+response[i].type_id+"'>"+response[i].type_name+"</a></li>");
				}
			}
		});
		...
		<main class="main-cont content mCustomScrollbar">
			<div class="page-wrap">
				...
				<section class="page-hd">
					<header>
						<h2 class="title">文章信息管理</h2>
						
						<div class="btn-drop-group">
							<button class="btn btn-secondary dropdown-toggle">新闻类型</button>
							<div  class="drop-list">
								<ul id="ul_type">
									<li><a href="${pageContext.request.contextPath}/b_newsManage/showNewsInfom.action">全部</a></li>
								</ul>
							</div>
						</div>
					</header>
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${showNewsInfo}" var="news">
					<tr class="cen">
							<td>${news.n_id}</td>
							<td>${news.n_title}</td>
							<td>${news.n_author}</td>
							<td>${news.n_time}</td>
							<td class="lt">${news.n_roundup}</td>
							<td>${news.type_name}</td>
							<c:if test="${news.n_state==1}">
								<td><font color="#16EE16">最新</font></td>
							</c:if>
							<c:if test="${news.n_state==2}">
								<td><font color="#1BAC83">最近</font></td>
							</c:if>
							<c:if test="${news.n_state==3}">
								<td><font color="#1B6CAC">普通</font></td>
							</c:if>
							<c:if test="${news.n_state==4}">
								<td><font color="#F3EC1A">陈旧</font></td>
							</c:if>
							<c:if test="${news.n_state==5}">
								<td><font color="#777070">过期</font></td>
							</c:if>
							<c:if test="${news.n_state==6}">
								<td><font color="#F3841C">待审核</font></td>
							</c:if>
							<td>
								<a href="${pageContext.request.contextPath}/b_newsManage/updateNews.action?n_id=${news.n_id}" href="${pageContext.request.contextPath}/b_newsManage/showDetails.action?n_id=${news.n_id}" title="修改" class="mr-5">修改</a>
								<a href="${pageContext.request.contextPath}/b_newsManage/showDetails.action?n_id=${news.n_id}" title="详情" class="mr-5">详情</a>
								<a href="${pageContext.request.contextPath}/b_newsManage/deleteRelease.action?n_id=${news.n_id}" title="删除">删除</a>
							</td>
						</tr>
					</c:forEach>
...
<script>
    ...
    $(".pagination").createPage({
        ...
        current:1,
        backFn:function(p){
           window.location.href="${pageContext.request.contextPath}/b_newsManage/showNewsInfom.action?begin="+p;
        }
    });
</script>
</body>
